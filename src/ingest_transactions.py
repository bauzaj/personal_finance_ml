...
def main(days_back: int = 90):
    log.info(
        f"Pulling last {days_back} days of transactions from Plaid (multiple institutions)"
    )
    plaid = PlaidClient()
    ...
    for t in tokens:
        ...
        log.info(
            f"  Pulled {len(transactions)} transactions from {t['institution_id']}"
        )

    log.info(
        f"Total: {len(all_transactions)} transactions across {len(tokens)} institutions"
    )

    log.info("Writing to Snowflake")
    with SnowflakeClient() as snow:
        merged = snow.insert_transactions(
            schema="RAW", table="PLAID_TRANSACTIONS", transactions=all_transactions
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--days", type=int, default=90, help="Days of transactions to pull"
    )
